...
func init() {
	android.RegisterModuleType("halcyon_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = halcyonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = halcyonExpandVariables(ctx, dep_file)
	...
	cmd := halcyonExpandVariables(ctx, String(g.properties.Cmd))
